...
import type { FlattenedItem, TreeItems, TreeItem } from "./types";
...
export function getProjection<T>(
  ...
  if (overId === "placeholder") {
    // Placeholder is a special case where we do not have
    const maxDepth = 0;
    return { depth: 0, maxDepth, minDepth: 0, parentId: null };
  }
  ...
  function getParentId(): null | UniqueIdentifier {
  ...
  function flatten(
    ...
    return items.reduce<FlattenedItem<T>[]>((acc, item, index) => {
      return [
        ...
        { ...JSON.parse(JSON.stringify(item)), parentId, depth, index },
 ...
 */
export function removeItem<T>(items: TreeItems<T>, id: UniqueIdentifier): TreeItems<T> {
  const newItems = [];
 ...
 */
export function setProperty<T extends keyof TreeItem, K>(
  items: TreeItems<K>,
  id: UniqueIdentifier,
  property: T,
 ...
 */
export function getChildCount<T>(items: TreeItems<T>, id: UniqueIdentifier): number {
